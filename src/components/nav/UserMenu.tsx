		...
		<>
			{status === "loading" ? (
				...
				<Menu as="div" className="relative inline-block text-left">
					...
					<Menu.Items className="absolute right-0 mt-2 w-48 origin-top-right rounded bg-light p-1 text-left text-dark dark:bg-dark dark:text-light">
						{status === "unauthenticated" ? (
							<UserMenuUnauthed />
						) : (
...
function UserMenuAuthed({ session }: { session: Session }) {
	...
	return (
		<Menu.Item>
			<button
				className="w-full rounded p-2 text-error hover:bg-error hover:text-white"
				onClick={() => signOut()}
			>
				Sign Out
			</button>
